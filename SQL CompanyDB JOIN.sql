...
SELECT 
    e.FNAME + ' ' + e.LNAME AS [Student Name],
    d.Dname AS ClassName
FROM Employee e
INNER JOIN Departments d ON e.DNUM = d.Dnum;
...
FROM Employee e
LEFT JOIN Departments d ON e.DNUM = d.Dnum
ORDER BY e.FNAME;

-- Task 3: RIGHT JOIN (All Right Table Records)
SELECT 
    d.Dname AS CustomerName, 
    p.Pnum AS OrderID       
FROM Projects p
RIGHT JOIN Departments d ON p.Dnum = d.Dnum
ORDER BY d.Dname;
...
SELECT 
    ...
    e.SSN AS SaleID
FROM Projects p
FULL OUTER JOIN Employee e ON p.Dnum = e.DNUM
ORDER BY p.Pname;
...
FROM Employee e
LEFT JOIN Employee m ON e.Superid = m.SSN
ORDER BY e.FNAME;

-- Task 6: CROSS JOIN (All Combinations)
SELECT 
    e.GENDER AS ColorName, 
    p.City AS SizeName
FROM Employee e
CROSS JOIN Projects p
ORDER BY e.GENDER, p.City;

-- Task 7: INNER JOIN with Multiple Tables
SELECT 
    e.FNAME + ' ' + e.LNAME AS Name, 
    d.Dname AS ClassName, 
    m.FNAME + ' ' + m.LNAME AS TeacherName
FROM Employee e
INNER JOIN Departments d ON e.DNUM = d.Dnum
INNER JOIN Employee m ON d.Manager_SSN = m.SSN
ORDER BY d.Dname;
...
FROM Employee e
LEFT JOIN Employee m ON e.Superid = m.SSN
LEFT JOIN Employee tm ON m.Superid = tm.SSN
ORDER BY e.FNAME;